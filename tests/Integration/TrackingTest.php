 ...
 *
 * Dispatching also runs the subscriber that resolves the current cart, which queries the database,
 * so this lives in the integration suite, which runs in the build job that has MySQL up.
 */
final class TrackingTest extends KernelTestCase
{
    ...
    {
        $this->eventDispatcher->dispatch((new Event(Events::PURCHASE))->setRevenue('DKK', 249.95));
    ...
    {
        $order = [];
        foreach ($this->eventDispatcher->getListeners(Event::class) as $listener) {
